...
| ID | Sev | Finding | Evidence |
|---|---|---|---|
| F13 | S1 | **The browser announces itself as `HeadlessChrome` with zero plugins.** `chromium.launch(headless=True)` uses Playwright's headless shell; the real-Chrome new-headless mode requires `channel="chromium"`. Verified locally with the app's exact launch args. | §6.3; https://playwright.dev/docs/browsers |
| F14 | S1 | **A brand-new browser per item fetch, no cookie persistence.** `_fetch_offers_for_asin` (Amazon), `BookfinderInlineSource._render`, and both metadata lookups each run `async_playwright()` → `launch()` → `close()`. Amazon sees a cold automated visitor ~120 times a day. | `amazon.py:225-268`; `bookfinder.py:127-156`; `metadata.py:369,484` |
...
- [ ] **T1.2 Delivery postcode pinning.** Only if T0.2 concluded the mechanism works. `SourceConfig.delivery_postcode: str | None = None` (validated `^[A-Z]{1,2}\d[A-Z\d]? ?\d[A-Z]{2}$` after upper-casing); `BrowserSession.pin_delivery_location(postcode)` runs once per context after launch when the profile has no pin cookie yet. Settings → Sources card gets a text field. Document in `.env.example` / README.
...
- [ ] **T2.5 Amazon flip diagnosis and fix.** After T0.2's conclusion: if pinning removes the flips, T1.2 is the fix and this task only adds the regression test on the pinned fixtures. If flips persist with pinning, add a `raw.delivery_text`-driven rule: when the promise text contains a conditional ("on orders over", "on your first order", "with Prime") treat it as **not free** and fall back to the cascade estimate with `shipping_is_estimate = true`. Decide from evidence; write the rule that the captures support.
...
- [ ] **T3.1 Stats restructure: candidates in SQL, selection in Python.** Tier 4 (migration).
...
- [ ] **T3.3 Sources API single query.** Replace `_last_run_for` loop with one `SELECT … ROW_NUMBER() OVER (PARTITION BY source ORDER BY started_at DESC)` query. Trivial; fold into T3.1's commit if convenient.
...
- [ ] **T4.3 Keepa date clamp and future-row guard.** `keepa_chart._DateCalib.__call__` clamps to `today`; `keepa_backfill` drops any extracted date `> today` and logs a count. Test with a frozen clock (`freeze_time` is already used in scenarios).
...
- [ ] **T4.5 Product alerts surface.** `api/alerts.py` returns a union of `Alert` and `ProductAlert` rows (`kind`, `item_kind`, `item_id`, `title`, message, fired_at, dismissed_at), with dismiss / dismiss-all covering both tables; `Alerts.tsx` links to `/products/:id` for product rows. Notification test endpoint unchanged.
...
- [ ] **T5.1 Shared item types and hooks.** `web/src/lib/item.ts`: `type ItemLike = { kind: "book" | "product"; id; title; imageUrl; stats; signal; … }` built from `BookOut` / `ProductOut`; `useItems(kind)`, `useItem(kind, id)` wrap the existing hooks. No behaviour change for books.
...
- [ ] **T6.5 Data-directory janitor (runtime clutter).** One APScheduler job `janitor` (daily 04:00 UTC, after the backup job) in a new module `src/book_alerter/janitor.py`, config section `janitor:` with `enabled: bool = True` and the limits below. It enforces, inside the mounted `data/` volume only:
