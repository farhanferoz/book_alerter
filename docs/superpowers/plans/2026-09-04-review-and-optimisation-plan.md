...
- [ ] **T6.2 `Signal` to `StrEnum`** 🟡 **PART DONE 2026-09-04** — the `Signal` StrEnum is committed (`ab5c6ed`, orchestrator-written; uppercase values kept deliberately because the frontend and `alerts.py` already compare them). **REMAINING: `stats.py` and `alerts.py` switching to the enum**, with the stats worker. (F25). Mechanical; ruff `PLR2004` clean.
