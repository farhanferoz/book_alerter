...
| F26 | **S1** | **Amazon silently serves the wrong page for an offer-listing request.** Discovered 2026-09-04 during T0.4. Two independent `/gp/offer-listing/<asin>?condition=all` requests returned a normal, fully-formed **dp page** instead — once for the *requested* ASIN (0 offer rows), and once with `<link rel="canonical">` pointing at a **different ASIN entirely** (`B0CYT8WL1G` → `B0DLSB1WWK`). **No `BOT_MARKERS` were tripped in either case**, so `_render_amazon_page` cannot detect it: nothing in the returned HTML looks like an error. The risk is not just a missed scrape — parsing a different ASIN's page attributes one product's prices to another. Fix: assert the response's canonical URL matches the request (offer-listing responses must carry `/gp/offer-listing/`, and the ASIN must match) and treat a mismatch as a source error. | T0.4 captures; `2026-09-04-wave0-probe-results.md` |
...
- [x] **T0.4 Product-page fixtures.** ✅ DONE 2026-09-04 — four product ASINs captured under `tests/fixtures/amazon/products/`, plus **Q3 answered: the Echo Dot's empty offer listing is genuine** (0 `#aod-offer` nodes, 1 `#aod-pinned-offer`; the dp path captures the offer correctly), so `parse_offer_listing` returning `[]` is right, not a miss. Original spec: Capture with T0.1: (a) a multi-seller non-Amazon-brand product with AOD rows, (b) a product with size/colour variants (`#twister` present), (c) a "currently unavailable" product, (d) a product with used offers, (e) the Echo Dot `B09B96TG33` (to settle F11). Store under `tests/fixtures/amazon/products/`. Record which of (a)–(e) exhibit which markers in the results file.

- [x] **T0.5 Bookfinder condition strings.** ✅ DONE 2026-09-04 — books 2 and 5 captured; 15 deduped cards yielded **zero unmapped grades** (`New`→new ×11, `Used - Like New`→used_vg ×4), every regex match agreeing with the structured `data-csa-c-condition`. **So the capture neither confirms nor refutes the plan's speculative mapping** — see D26 for how T2.6 proceeds. Original spec: From the prod copy, list the distinct raw condition texts behind `condition = unknown` (they are not stored — so instead capture two Bookfinder pages for books 2 and 5 with T0.1-style script extended with `--source bookfinder`) and record the unmapped grade strings.
...
- [x] **T1.3 Per-item challenge handling and backoff.** ✅ **DONE 2026-09-04** — schema + migration 0022 (`f9b32ba`, Tier 4, property test written first and watched fail), scheduler behaviour (`0dbb2a2`-ish), and T6.1's `challenged` figure switched from proxy to exact. **One correction to this task's premise:** it says "today only run-level exceptions count" — not so, a run with zero successes already counted. The real gap was that ONE succeeding item RESET the consecutive-error counter, so backoff never engaged while the source was plainly blocking us (the observed production state: 10 of 13 books carrying a challenge error, runs still finishing `partial`). A run with ≥50% of attempted items challenged now counts as a consecutive error regardless of successes. `items_challenged` counts items still blocked AFTER the retry, so an item that recovers is a success and is not counted. 4 tests, each verified red-then-green. In `_run_kind_for_source._one`: on a `SourceError` whose message contains the bot-challenge text, sleep `random.uniform(20, 40)` s and retry **once** with a fresh page in the same context. Count item-level challenge failures; if ≥50% of attempted items in a run were challenged, treat the run as a consecutive error for `_apply_backoff` (today only run-level exceptions count). Extend `SourceRun` with `items_challenged: int` (migration, Tier 4).
Verify: scheduler integration test with a stub source raising the challenge error on first call.

- [x] **T1.4 Browser concurrency cap and staggering.** ✅ **DONE 2026-09-04** — `scheduler.max_concurrent_browsers` (default 2, floor enforced by the model), a process-wide semaphore held for the session's whole open lifetime, and 0/15/30/45 default schedules. **Two design points the plan left open, settled here:** the slot is taken AFTER the D24 per-profile lock, never before, which makes deadlock unreachable and stops a session queued on its own profile from holding a browser slot; and because `asyncio.Semaphore` cannot be resized, a reload builds a new one while `close()` releases the object `start()` actually took — otherwise a mid-session reload gives the new semaphore a phantom slot. 5 tests, 2 verified red-then-green (the failure path matters: bot challenges make failed launches routine, and leaking one slot per failure tightens the cap until nothing can open a browser). New `SchedulerConfig.max_concurrent_browsers: int = 2` (top-level `scheduler:` section in `Config`, default preserves current behaviour for `≤2` sources). A process-wide `asyncio.Semaphore` wraps `BrowserSession.__aenter__`. Default schedules in `_default_sources()` staggered: wob `0 */6`, bookfinder `15 */6`, amazon `30 */6`, amazon_uk_product `45 */6` (existing user config files are untouched; README notes the recommendation).
Verify: config round-trip test; scheduler test that two sources with the cap at 1 do not overlap `prepare()`.

- [x] **T1.5 Diagnostic capture.** ✅ DONE 2026-09-04 (`ec085d7`) — `delivery_text` captured for T2.5, debug dumps under `data/debug/<source>/` reusing `JanitorConfig.debug_keep_files` rather than a second limit, **The F26 canonical-URL guard is NOT in this commit** — I ticked it prematurely and corrected the record; it is still outstanding with the browser worker. Original spec: `ObservationCandidate` gains `delivery_text: str | None = None` (Amazon: the `.aod-delivery-promise` text or dp delivery block text; Bookfinder: the shipping label text). Persisted inside `raw` (no schema change). On a bot challenge or an unrecognised layout, `_render_amazon_page` writes the HTML to `data/debug/<source>/<utc-timestamp>.html`, keeping the newest 20 files per source. Directory is gitignored/dockerignored and inside the mounted `data/` volume.
...
- [x] **T2.2 Prime toggle (backend).** ✅ **DONE 2026-09-04** (`67c3ccf`), root-verified at HEAD: `effective_shipping()` in `stats.py`, `RecommendationConfig.amazon_prime` (default False), and `BookStatsOut` exposing both `shipping_is_estimate` and `prime_applied`. **Carried S1/D34 with it** — the target-price comparisons in `compute_signal` and `alerts.py` moved to `current_effective_total_minor` in the same change, because they interact through `prev_signal` and could not be split. Verified by re-running the reviewer's probe: target £8.00 against an effective £10.79 no longer fires `TARGET_HIT`. `RecommendationConfig.amazon_prime: bool = False`. In `stats.py`, a single function `effective_shipping(source, seller, shipping_minor, *, prime: bool, cascade) -> tuple[int, bool]` returning `(pence, is_estimate)`: if `prime` and `seller_class(seller) == amazon_fulfilled` and source in `{"amazon", "amazon_uk_product"}` → `(0, False)`; elif observed → `(observed, False)`; else → `(cascade(...), True)`. Every place that adds shipping (window totals, current-best ranking after T3.1, alert message) goes through it. `BookStats` gains `shipping_is_estimate: bool` and `prime_applied: bool`; `BookStatsOut` mirrors them.
...
- [x] **T2.3 Prime toggle (UI).** ✅ **DONE 2026-09-04** (`02bdc1e`), root-verified: the `Switch` wired through the existing PUT-with-diff pattern, and `prime_applied`/`shipping_is_estimate` consumed **directly from the backend with no client-side re-derivation** (D10). **A real bug was caught by the worker's manual check, not by the CLI gate:** `SnapshotCard`'s headline still showed the raw total while its caption said "Prime (free delivery)" — an on-screen contradiction (£19.80 headline vs a true £17.00). Verified live through the real UI against a production copy, toggling both ways and confirming all three surfaces agree. Settings → Recommendation: a labelled `Switch` "I have Amazon Prime (treat Amazon-fulfilled delivery as free)" using the existing PATCH pattern in `web/src/pages/settings/Recommendation.tsx` (`model_dump(exclude_unset=True)` path already accepts new keys). Offer rows in `SourceBreakdown`, `SnapshotCard`, `PercentileChart` show "Prime" when `prime_applied` and "est." when `shipping_is_estimate`. Regenerate `web/src/api/schema.ts` (`npm run gen:api` against a running backend).
...
- [x] **T2.4 Unknown shipping never ranks as free.** ✅ **DONE 2026-09-04** — the deferred `shipping_is_estimate is True` assertion landed with T2.2 (`tests/integration/test_unknown_shipping_ranking.py:206`), which is the field it was waiting on. Previously: 🟡 **RANKING HALF DONE 2026-09-04** — `tests/integration/test_unknown_shipping_ranking.py`, 4 tests, verified red-then-green by reintroducing F3. **REMAINING: the `shipping_is_estimate = true` assertion**, which needs the field T2.2 adds — fold it in there. Original spec: Delivered by T3.1's Python current-best selection (rank on effective total). Add a regression test: two live offers, same price, one NULL shipping and one 280 → the 280 one wins only if the cascade estimate for the NULL row exceeds 280; the response carries `shipping_is_estimate = true` when the NULL row wins.

- [x] **T2.5 Amazon flip fix — conditional delivery promise.** ✅ **DONE 2026-09-04 (`e27b60c`) — this closes the S1 bug the whole plan was written around.** `_conditional_free_shipping_to_unknown()` turns a free-reading price with a conditional promise into `None`, so `effective_shipping` substitutes the cascade estimate instead of £0.00. Applied at both the dp and offer-listing paths. **Root-verified independently on the real `B0F3NVWM37` capture, all 10 genuine rows in one page: 8 conditional → `None`, the 1 genuinely-free row still `0`, the 1 paid row still `299`.** Marker set is evidence-based: only `on your first order` is matched — the plan's other two guesses (`on orders over`, `with Prime`) appear nowhere near a delivery line in any capture on file, and the omission is documented in the code so it does not read as an oversight. Original spec: ⚠️ **T0.2 has decided this: take the second branch below; it is now the primary fix, not a fallback.** The marker observed live is `on your first order` (full text `FREE delivery <dates> on your first order to UK or Ireland`). This task now hard-depends on **T1.5** (which captures `delivery_text`). Add a `raw.delivery_text`-driven rule: when the promise text contains a conditional ("on orders over", "on your first order", "with Prime") treat it as **not free** and fall back to the cascade estimate with `shipping_is_estimate = true`. Decide from evidence; write the rule that the captures support.
Verify: parser unit tests on the T0.2 captures.

- [x] **T2.6 Bookfinder condition mapping.** ✅ DONE 2026-09-04 (`c25b455`) — per D26 the mapping ships as a conventional best-effort (T0.5 found no live evidence for these strings), with `Ex-library` deliberately excluded so it keeps its accompanying grade. **The load-bearing half is the diagnostic**: an unmapped grade now logs `condition_normalizers.grade_unmapped` with the raw text, so the real distribution behind the 2,738 production `unknown` rows becomes visible instead of being discarded. Original spec: Extend `condition_normalizers.condition_from_grade_text` with the strings T0.5 found (e.g. "Fine", "Near Fine", "As New" → `used_vg`; "Fair" → `used_acceptable`; "Ex-library" keeps its grade). Fixture-based tests.

- [x] **T2.7 dp seller attribution.** ✅ DONE 2026-09-04 (`ec085d7`). Original spec: `_extract_dp_seller` returns `None` (not `"Amazon"`) when `#merchant-info` is absent; downstream treats `None` as third-party for the cascade. Test on the `9780747532699-uk-dp.html` fixture.
...
- [x] **T3.2 Heartbeat compaction.** ✅ **DONE 2026-09-04** (`0cd4a16`, one atomic commit — `_persist` and `keepa_backfill` both reference a column the old model lacked, so a partial commit would have been a broken tree). Shipped as migration `0021_heartbeat_compaction`. Root-verified independently of the worker: 62 passed across the 8 affected suites, and `git show --stat` confirms nothing outside scope came along. Worker's evidence, on production copies: 90,172 → 12,337 rows with the canonical count unchanged; `last_seen_at` matches the old `buyable_last_seen` formula for all 4,304 non-keepa canonical rows (0 mismatches); FK check, `integrity_check` and a downgrade→upgrade round-trip all clean. **D23's ≤0.35 s gate is CLOSED at 0.059–0.068 s**; `smoke_check.py` 12/12 with `GET /api/books` at 63.5 ms (2101 ms → 453 ms after T3.1 → 63.5 ms here, ~33× end to end). Tier 4 (migration).
...
- [x] **T3.4 Medians snapshot cache.** ✅ **DONE 2026-09-04** (`e2c0aae`), verified at HEAD: `MediansCache` on `app.state`, consumed by both `api/books.py` and `api/products.py`, invalidated from `scheduler._persist` via the `app_state` hook added for the janitor rather than a second parallel one. `source_seller_global_shipping_medians` cached on `app.state` with a 60 s TTL, invalidated by the scheduler after each `_persist` batch. Removes the 0.12 s scan from every dashboard render.

### Wave 4 — products reliability

Depends on Wave 1 (browser session) for T4.1's background refresh.

- [x] **T4.1 Add-product never blocks on Amazon.** ✅ **DONE 2026-09-04** (`66097d9`, 17 files, one atomic commit because `scheduler.py` references a column in `models.py`). Root-verified at HEAD: migration **0023** (renumbered from the plan's 0022, which T1.3 had taken) with its backfill formula property-tested **first** and watched fail on a swapped branch (D17); `ProductCreate.title` optional with an `Amazon product <asin>` placeholder + PENDING; `metadata_refresh` job registered with exponential backoff to 6 attempts; `_persist` resolving a PENDING row from the scraper's own dp parse so a product rarely waits for a job tick; `parse_dp` populating `item_title`/`item_image_url`. Migration chain single-headed. 36 tests green across the four affected suites; `tsc` clean. Also folded in a **missed S4 site** — `lib/item.ts`'s `applyItemFilters` was a fourth copy of the raw-total sort that the original S4 fix did not catch. Backend: `ProductCreate.title` becomes optional; when absent the row is created with `title = f"Amazon product {asin}"` and `metadata_status = "pending"` (new column, migration `0022_product_metadata_status`, values `pending|ok|failed` as a `StrEnum` in `enums.py`). A background job (`metadata_refresh` in `scheduler.py`, every 30 min, plus immediately after create) retries `fetch_amazon_uk_product_metadata` through `BrowserSession` with exponential backoff up to 6 attempts, then marks `failed`. The product scraper also fills title/image on its first successful dp parse (`parse_dp` already sees `#productTitle`; extend `ObservationCandidate` with optional `item_title` / `item_image_url` and let `_persist` update a pending product). Frontend: Confirm enabled as soon as `looksLikeAsinOrUrl` passes; preview shows the lookup result when it arrives, otherwise "Details will be filled in after the first scrape"; a pending badge on the product row.
Verify: API test creating a product with `{asin_or_url}` only; scheduler test for the refresh job; FE build.

- [x] **T4.2 Product page parser hardening.** ✅ **DONE 2026-09-04** (`98f0cf6`), root-verified: `tests/integration/sources/test_amazon_product_fixtures.py`, 10 tests, 0.35 s, all green. §8 gate cleared — all 8 product `.html` fixtures are now loaded by a test (checked file-by-file, none unreferenced). Covers B0F3NVWM37 (10 AOD rows + first-order-promo rows → unknown shipping), B0CYT8WL1G (the variant page — **correction to this plan's marker guidance: `#twister_feature_div` does NOT discriminate either.** A DOM-node audit of all 8 fixtures found it present on all 4 ASINs, dp and aod alike; `#inline-twister-row-color_name`/`-size_name` do vary but also fire on B09B96TG33, which genuinely has colour variants. **No marker-gated variant detection was built, and none is possible from these markers** — Amazon resolves the selected variant server-side, so `parse_dp` reads the right buy box with no branching; the test pins that outcome directly. Also covers the **F26 wrong-ASIN** case), B0GX54WT36 (unavailable → `[]`, no raise, both dp and aod), B09B96TG33 (genuine single-seller empty listing). The 8 `.json` sidecars are capture-provenance evidence from `scripts/capture_amazon_fixture.py`, not parser inputs, so §8 does not bind them — see the audit note below. ⚠️ **MANDATORY, not optional** — audit 2026-09-04 found **7 committed product fixtures that no test loads**, which violates §8 ("No fixture without a test that loads it"). T4.2 is the task that consumes them: `B09B96TG33-uk-aod`, `B0CYT8WL1G-uk-{aod,dp}`, `B0F3NVWM37-uk-{aod,dp}`, `B0GX54WT36-uk-{aod,dp}`. Either every one gets a test naming its expected offers, or the unused ones are deleted — shipping dead fixtures is not an option. Marker guide from T0.4: `B0F3NVWM37` has 10 real AOD rows (multi-seller + used); `B0CYT8WL1G` is the variant page (use `#twister_feature_div`, **not** `#twister`); `B0GX54WT36` is currently-unavailable (`#outOfStockBuyBox_feature_div`, no price, no add-to-cart); `B09B96TG33` is the genuine single-seller empty listing. Original spec: Using T0.4 fixtures: variant pages (⚠️ **T0.4 correction: the plan's `#twister` marker is STALE — it matches 0 nodes on both a genuine variant page and a non-variant page. Use `#twister_feature_div` or `#inline-twister-row-color_name` / `#inline-twister-row-size_name`**) — read the selected variant's buy box only and record `variant_asin` if the URL ASIN differs; "currently unavailable" — return `[]` without raising (page markers present); single-seller pages — dp-only path must not raise on an AOD page with zero rows (already tolerated; add the fixture test); used offers honour `track_used`. Add `tests/integration/sources/test_amazon_product_fixtures.py`.
...
- [x] **T4.4 Keepa-only history caveat.** ✅ **DONE 2026-09-04** — backend (`26d6c21`) plus the `SignalCard` pill (`02bdc1e`), whose wording matches `dispatcher.py`'s, adapted only for standalone-sentence grammar. **Known coverage gap, flagged not hidden:** no book in current production data has `live_observation_count == 0`, so the pill was verified by reading the condition against the tested backend guard rather than seen on real data. Previously: 🟡 **BACKEND DONE 2026-09-04** — `BookStats.live_observation_count` counted off the window rows already in hand (no extra query), exposed on `BookStatsOut` (products reuse the same model, so both kinds get it), and the alert-message caveat is live in `dispatcher.py`, guarded on `observation_count > 0` so it never claims Keepa history for an item with none. 3 tests, verified red-then-green. **REMAINING: the FE signal pill**, with the Prime-UI worker — it must match the dispatcher's wording verbatim, and read as provenance context rather than an error, since the signal itself is legitimate (D16). `BookStats` gains `live_observation_count: int` (non-Keepa canonical rows in the window). `compute_signal` still fires (decision: Keepa history is valid history), but the FE shows "Based on Keepa history only — no live offer yet" on the signal pill when `live_observation_count == 0`. Alert messages carry the same suffix.

- [x] **T4.5 Product alerts surface.** ✅ DONE 2026-09-04 (commit `f75c7a1`) — union feed over both tables driven by the existing `_AlertModels` registry (no per-kind duplication); local `AlertKind` Literal replaced by the `StrEnum`; alerts addressed by `(item_kind, id)` because ids collide across tables (regression test seeds colliding ids); rows carry their own title so the client-side `useBooks` lookup is gone from both page and sidebar. 17 alerts tests green; FE tsc/eslint/build green. **Audit follow-ups (2026-09-04):** scenario 07 now asserts the alert surfaces via `GET /api/alerts` and dismisses by kind (the clause was initially missed). ⚠️ **Deviation from the spec's "Notification test endpoint unchanged"**: `api/notifications.py` *did* need a change — it called `AlertOut.from_alert()` with the old signature and raised `TypeError`. Endpoint behaviour is unchanged; only the call site moved (`7902701`). Original spec: `api/alerts.py` returns a union of `Alert` and `ProductAlert` rows (`kind`, `item_kind`, `item_id`, `title`, message, fired_at, dismissed_at), with dismiss / dismiss-all covering both tables; `Alerts.tsx` links to `/products/:id` for product rows. Notification test endpoint unchanged.
...
- [x] **T5.2 Products dashboard parity.** ✅ DONE 2026-09-04 (`57848aa`) — took the rename path (`BookRowMenu` → `ItemRowMenu`) after judging it mechanical; `columns.tsx`, `MiniBars`, `signal.tsx`, `CoverImage` generalised over `Item`; `ProductsDashboard.tsx` shrank from 125 lines of near-duplicate to reuse. Root-verified: tsc and eslint both exit 0. Original spec: Reuse `BookTable`, `columns.tsx`, `MiniBars`, `SignalPill`, `BookFilters`, `BookRowMenu` via `ItemLike` (rename to `ItemTable` etc. only if the rename is mechanical; otherwise wrap). Products dashboard gains signal pill, mini bars, current-best with source/seller, filters, row menu (refetch / archive / mute).

- [x] **T5.3 Product detail parity.** ✅ DONE 2026-09-04 (`f15acb8`) — all 9 detail cards reused via `Item`; `ProductDetail.tsx` shrank 346 lines. Backend contract **verified not assumed**: `ObservationsPage`/`ProductObservationsPage` are structurally identical and differ only in an FK name no component reads. Two self-caught bugs: post-delete navigation pointed at `/books`, a route that does not exist (books live at `/`); and `ProductDetail` was not lazy-loaded, pulling Recharts into the main chunk (**604 → 973 kB**, fixed to **599 kB** — root-verified, now smaller than the pre-Wave-5 baseline). Original spec: `HeaderCard`, `SnapshotCard`, `SignalCard`, `PercentileChart`, `HistoryChart`, `SourceBreakdown`, `KeepaChart`, `SettingsPanel`, `ActionBar` accept `ItemLike` + the `/api/products/...` endpoints. Backend: confirm `GET /api/products/{id}/observations` and `/stats` already expose everything the book components read (`BookStatsOut` is shared — yes; observations page shape — verify `ProductObservationsPage` matches `ObservationsPage`, align if not).

- [x] **T5.4 Products settings parity.** ✅ DONE 2026-09-04 (`f15acb8`) — shared `SettingsPanel` plus the products-only `track_used` toggle, verified with a real PATCH round-trip. Original spec: Per-product alert-kind disable, mute-until, percentile threshold/window, target price, notes — all already in `ProductPatch`; expose in the shared `SettingsPanel`.

- [x] **T5.5 Products in global surfaces.** ✅ **DONE 2026-09-04** (`1925c22`) — nav badge in `web/src/layout/AppShell.tsx` reuses `useAlerts({dismissed:false})`, which already unions books and products via `item_kind` (T4.5), so it counted both kinds from the first render; `web/src/components/settings/SourceCard.tsx` surfaces last-run attempted/succeeded on the card itself. App shell nav badge counts include product alerts; Settings → Sources shows per-kind attempted/succeeded for `amazon_uk_product`.
...
- [x] **T6.1 Scrape-health summary.** ✅ **DONE 2026-09-04** (`c0cb38a` backend + `c78293c` banner), root-verified: `schema.ts` regenerated (not hand-edited), banner in `web/src/pages/Dashboard.tsx`, and `tsc`/`eslint`/`npm run build` all clean. Two deliberate improvements on the spec: worded **"in the last 24 hours"** rather than the plan's "in the last run", because that is the window `last_24h` actually covers; and the duplicated source-label map was promoted from `HistoryChart.tsx` into `lib/format.ts::sourceDisplayName` so banner and chart legend share one mapping. The over-broad caveat on `challenged` is carried in the UI and goes away when T1.3 lands `items_challenged`. Previously: 🟡 **BACKEND DONE** (`c0cb38a`, written by the orchestrator because the single-writer guard held the file) — `last_24h {attempted, succeeded, challenged}` in one grouped query; `challenged` is documented as an over-broad proxy until T1.3's `items_challenged` lands. The query-count test now asserts the count is identical for 4 and 12 sources rather than a literal 1. **REMAINING: the dashboard banner** (`web/`). Original spec: `GET /api/sources` adds `last_24h: {attempted, succeeded, challenged}` per source (one query over `SourceRun`); the dashboard shows a banner "Amazon blocked N of M items in the last run" when `challenged > 0`.

- [x] **T6.2 `Signal` to `StrEnum`** ✅ **DONE 2026-09-04** (`ab5c6ed` + `70100bb`), verified at HEAD: `stats.py` and `alerts.py` both use the enum and **no bare signal string literal remains in either** (checked by grep for the four values). Previously: 🟡 **PART DONE 2026-09-04** — the `Signal` StrEnum is committed (`ab5c6ed`, orchestrator-written; uppercase values kept deliberately because the frontend and `alerts.py` already compare them). **REMAINING: `stats.py` and `alerts.py` switching to the enum**, with the stats worker. (F25). Mechanical; ruff `PLR2004` clean.

- [x] **T6.3 Periodic Keepa refresh** ✅ **DONE 2026-09-04**, ships **default-off** as specified. **Correction to this task's premise:** it says "dedup by date already exists" — it did not. The only guard was "skip if ANY keepa row exists for this item"; there was no per-date check anywhere, so a periodic re-run would either do nothing (guard intact) or duplicate the whole history weekly (guard removed). `backfill_blocking` now takes `refresh=True`, which skips the coarse guard and dedups on (seller, condition, **date**) — date, not datetime, because SQLite returns naive and `observed_at_to_datetime` returns UTC-aware, so a datetime comparison silently never matches. The dedup test caught that before it shipped. **The "decide at implementation time whether Keepa tolerates 13+ requests/week" question is NOT answered** — it cannot be answered from here without probing someone else's free service, so it is recorded as unmeasured in the config docstring, the tick function and the README, and the switch belongs to whoever is willing to find out. (F24) Optional weekly job re-running the backfill for active items (dedup by date already exists). Decide at implementation time whether Keepa's PNG endpoint tolerates 13+ requests/week; default off.

- [x] **T6.4 Docs.** ✅ **DONE 2026-09-04** — all three deferred items now written: the manual `VACUUM` step (with the measured row-count drop and why SQLite will not reclaim the space on its own), the staggered schedules plus the `max_concurrent_browsers` cap, and the Prime toggle (stating that it is applied at read time so toggling never rewrites history, and that it covers Amazon-fulfilled offers only). Previously: 🟡 **PART DONE 2026-09-04** — README now covers the data directory and its retention rules, the scraping/anti-bot posture (what each of the three settings actually fixes, and that the persistent profile is a data-correctness measure), that postcode pinning is unsupported and why, compressed-backup restore, and the `docker logs --since` / `--tail` gotcha (F23). **REMAINING, each blocked on its feature landing: the Prime toggle (T2.3), staggered schedules (T1.4), and the manual `VACUUM` step (T3.2).** Original spec: README: Prime toggle, postcode pinning, browser profiles directory, staggered schedules, `VACUUM` after T3.2, log-file note (F23: recommend `logging.options.mode: non-blocking` only if measured to help; otherwise document `docker logs --tail`).

- [x] **T6.5 Data-directory janitor (runtime clutter).** ✅ **DONE 2026-09-04** — the remaining APScheduler registration landed once T3.2 freed `scheduler.py`; 4 tests, each verified red-then-green by breaking the registration and the `data_dir` derivation in turn. Previously: 🟡 **MOSTLY DONE 2026-09-04** (commit `67f09ed`): `src/book_alerter/janitor.py` with all five sweeps, `JanitorConfig` (every limit a config value), `janitor_last_run_at` on `/api/health`, `janitor_tick()` entry point, 18 tests green. **Verify step done against real NAS data 2026-09-04:** ran against a copy of the live `data/` artefacts — **87.8% of the sampled bytes freed in 0.84 s**; one real 25.5 MB backup compressed to 2.9 MB (**88.6%**), so the NAS's 245.6 MB of backups (82% of its 298 MB data dir) should fall to ~28 MB. Removals audited row by row: 12 stale keepa PNGs (38–104 days old against a 24 h TTL), 1 genuinely orphaned keepa chart and 1 orphaned cover, both for an item no longer tracked. No tracked item lost anything it still needs. **REMAINING: register the APScheduler job** — one call to `janitor_tick()` in `scheduler.py`, deferred because `scheduler.py` is being restructured by T1.1 concurrently. Original spec: One APScheduler job `janitor` (daily 04:00 UTC, after the backup job) in a new module `src/book_alerter/janitor.py`, config section `janitor:` with `enabled: bool = True` and the limits below. It enforces, inside the mounted `data/` volume only:
...
- [x] **T6.6 Repository tidy-up.** ✅ **DONE 2026-09-04** (`3a70353`), root-verified: both legacy fixtures renamed to the dated form with dates derived from git history rather than invented (`9780241638194-uk-offer-listing-2026-05-16.html`, `9780747532699-uk-dp-2026-05-14.html`), all call sites updated, and a file-by-file sweep confirms **no fixture is left unreferenced**. The `.gitignore` entries, `cli_bins` removal, `scripts/README.md` and `data/amazon_dbg.png` items were already satisfied and were re-confirmed rather than redone. Add `.hypothesis/` and `data/browser-profiles/`, `data/debug/` to `.gitignore` (the first is only in `.dockerignore` today); remove the dead `cli_bins/**` ignore entries (the Go CLI path was abandoned in May); delete `data/amazon_dbg.png` from any clone that has it (it is git-ignored but is clutter). Move `scripts/capture_amazon_fixture.py` (T0.1) and `scripts/bench_stats.py` (T3.1) under a `scripts/` README listing every script and when to run it. Confirm `tests/fixtures/amazon/` follows the layout `<asin>-uk-<kind>-<date>.html` for books and `products/<asin>-uk-<kind>-<date>.html` for products; rename the two legacy `-real` / `-uk-dp` files to the dated form and update the tests that load them.
Verify: `git status --short` empty after commit; `uv run pytest -q`.

- [x] **T6.8 Write-containment e2e assertion.** ✅ DONE 2026-09-04 (`7251bda`) — and it **caught a real leak on its very first run, unprompted**: Chromium's first launch seeds ~37 paths under `/home/pwuser/{.cache,.config,.local}` outside `/app/data`. Verified bounded rather than growing (a second scheduler run added zero further paths) and excluded with that evidence recorded — see D27. Negative case demonstrated separately: a deliberate `touch /app/stray-leak.txt` made the assertion fail naming exactly that path, and pass again once removed. Original spec: (Added 2026-09-04 during execution: plan §8 already *requires* this — "the Docker e2e test asserts that the only new paths after a scheduler run are under `/app/data`" — but no such assertion exists, so the standard was unenforced.) Extend `tests/e2e/test_smoke.py`: snapshot the container filesystem before and after a scheduler run and assert every newly-created path lives under `/app/data`. Exclude paths the runtime legitimately churns (`/tmp`, `/proc`, `/sys`, `/dev`, Python bytecode caches) and say in a comment why each exclusion is safe. The test must **fail** if a source writes a debug dump, a browser profile, or a cache anywhere else in the image.
Verify: the assertion is seen to fail when a stray write outside `/app/data` is introduced deliberately, then pass once removed — a containment check never seen failing is not evidence.
