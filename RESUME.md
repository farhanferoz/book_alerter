...
**Branch:** `master` (no worktree, linear chain). 50 commits beyond the prior MVP-complete head (`bd4ffa5`): 27 pre-products + 9 products + 1 cleanup + 1 source-default + 8 bug-sweep + 3 post-sweep follow-ups + 1 final docs.
**Last update:** 2026-05-24, display-layer sweep on top of the NAS deployment — three rounds of "looks wrong on the live page" screenshots fixed: (1) `current_best`/breakdown ranked by latest-seen offer + Amazon clickout repaired (migration `0018`); (2) `current_best`/`/observations` surface the LATEST sighting's url + `last_seen`, not the frozen first-seen canonical (migration `0019`); (3) history chart plots the cheapest *live* offer per source (interval envelope), killing the phantom £25/£23 spikes (FE-only, `6d8139a`). All verified in a real browser against prod-DB copies before deploy. Tests green: **426 unit/integration + 3 skipped + 7/7 scenarios + ruff clean + FE TS+ESLint+build clean + alembic head `0019_views_current_best_latest_url`**. Live deployment at **http://100.115.46.9:8090** (Tailscale-only) running `ghcr.io/farhanferoz/book_alerter:latest` (image built from `6d8139a`).
...
- ~~SourceBreakdown "Current best" highlight pinned the wrong row~~ — shipped in `48aba42`: book 4 (1929) snapshot said `WOB · used_vg · £16.00` but the breakdown highlighted the £16.80 row (same seller/condition, different price — WoB lists multiple copies under one product URL). The FE match key was `(url, source, condition, seller)` after `603f34b` over-corrected by dropping `total_minor`. Restored: the original concern (penny drift across scrapes) doesn't apply because the snapshot and observations array read from the same fresh view/table.
- ~~Price history chart very busy / noisy after 3 May~~ — shipped in `1253e44` + `d70d874`: two compounding chart-render bugs. (a) `buildSeries` did `row[key] = o.total_minor` which overwrote with whichever marketplace seller came last in iteration order — so as seller order shuffled between scrapes the rendered line jumped randomly between sellers' prices. Pre-2026-05-03 the chart was smooth because the data was Keepa-backfill (one row/day, no marketplace flux). (b) The chart rendered one line per `(source · condition)` — 5+ overlapping series in a small viewport. Fix (a): aggregate by MIN(total) per `(ts, series_key)`. Fix (b): collapse to two condition families — `New` (condition=new) and `Used` (condition in {used_vg, used_g, used_acceptable}) — matching Keepa's own chart shape.
